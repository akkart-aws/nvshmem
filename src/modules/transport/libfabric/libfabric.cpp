...
static int nvshmemt_libfabric_gdr_process_completion(nvshmem_transport_t transport,
    ...
    if (entry->flags & FI_REMOTE_CQ_DATA) {
        ...
        if (NVSHMEMT_LIBFABRIC_IMM_PUT_SIGNAL_SEQ == imm_header ||
            ...
            NVSHMEM_TRACE_RECEIVER_COMPLETION_RMA(ep->domain_index, (int)imm_header);
            goto out;
            ...
            NVSHMEM_TRACE_SENDER_COMPLETION_AMO_ACK(ep->domain_index, (uint64_t)*addr);
            goto out;
        } else if (NVSHMEMT_LIBFABRIC_IMM_STANDALONE_PUT_ACK == imm_header) {
            nvshmemt_libfabric_put_signal_ack_completion(state, ep, entry, *addr);
            NVSHMEM_TRACE_SENDER_COMPLETION_SIGNAL_ACK(ep->domain_index, (uint64_t)*addr);
            goto out;
        } else {
    ...
    if (entry->flags & FI_SEND) {
        if (op->type == NVSHMEMT_LIBFABRIC_MATCH) {
            NVSHMEM_TRACE_SENDER_COMPLETION_SIGNAL(ep->domain_index, (uint64_t)entry->op_context);
        } else if (op->type == NVSHMEMT_LIBFABRIC_SEND) {
            NVSHMEM_TRACE_SENDER_COMPLETION_AMO(ep->domain_index, (uint64_t)entry->op_context);
        } else if (op->type == NVSHMEMT_LIBFABRIC_ACK) {
            NVSHMEM_TRACE_RECEIVER_COMPLETION_AMO_RESPONSE(ep->domain_index, (uint64_t)entry->op_context);
        }
        state->op_queue[ep->domain_index]->putToSend(op);
        ep->completed_ops++;
    } else if (entry->flags & FI_RMA) {
        if (op->type == NVSHMEMT_LIBFABRIC_RMA) {
            NVSHMEM_TRACE_SENDER_COMPLETION_RMA(ep->domain_index, (uint64_t)entry->op_context);
        } else if (op->type == NVSHMEMT_LIBFABRIC_SIGNAL_ACK_WRITE) {
            NVSHMEM_TRACE_RECEIVER_COMPLETION_SIGNAL_ACK(ep->domain_index, (uint64_t)entry->op_context);
        } else if (op->type == NVSHMEMT_LIBFABRIC_AMO_ACK_WRITE) {
            NVSHMEM_TRACE_RECEIVER_COMPLETION_AMO_ACK(ep->domain_index, (uint64_t)entry->op_context);
        }
        ...
        status = nvshmemt_libfabric_put_signal_completion(transport, ep, entry, addr);
        NVSHMEM_TRACE_RECEIVER_COMPLETION_SIGNAL(ep->domain_index, addr ? (uint64_t)*addr : 0, 0);
        ...
        if (op->type == NVSHMEMT_LIBFABRIC_ACK) {
            ...
            NVSHMEM_TRACE_RECEIVER_COMPLETION_AMO(ep->domain_index, (uint64_t)entry->op_context);
...
static int nvshmemt_libfabric_auto_proxy_progress(nvshmem_transport_t transport) {
    return nvshmemt_libfabric_progress(transport, NVSHMEMT_LIBFABRIC_PROXY_EP_IDX);
}
...
static void *nvshmemt_libfabric_signal_delivery_thread(void *arg) {
    ...
    /* Pin to same NUMA node as the spawning thread (shares LLC) */
    if (state->spawner_cpu >= 0) {
        cpu_set_t cpuset;
        CPU_ZERO(&cpuset);
        CPU_SET(state->spawner_cpu, &cpuset);
...
static int nvshmemt_libfabric_progress(nvshmem_transport_t transport, int qp_index) {
    ...
    if (libfabric_state->provider == NVSHMEMT_LIBFABRIC_PROVIDER_EFA) {
        ...
        /* Dequeue from op_queue, push to work_queue for Thread B */
...
int gdrcopy_amo_ack(nvshmem_transport_t transport, nvshmemt_libfabric_endpoint_t *ep,
    ...
    ep->submitted_ops++;
    if (ack_header == NVSHMEMT_LIBFABRIC_IMM_STANDALONE_PUT_ACK) {
        NVSHMEM_TRACE_RECEIVER_POST_SIGNAL_ACK(pe, ep->domain_index, sequence_count);
    } else {
        NVSHMEM_TRACE_RECEIVER_POST_AMO_ACK(pe, ep->domain_index, sequence_count);
    }
...
int perform_gdrcopy_amo(nvshmem_transport_t transport, nvshmemt_libfabric_gdr_op_ctx_t *op,
    ...
    volatile T *ptr;
    int status = 0;
    NVSHMEM_TRACE_RECEIVER_GDRCOPY_START(op->ep->domain_index, received_op->op, op->type == NVSHMEMT_LIBFABRIC_MATCH ? 1 : 0);
...
out:
    NVSHMEM_TRACE_RECEIVER_GDRCOPY_END(op->ep->domain_index, status);
    return status;
}
...
int nvshmemt_libfabric_put_signal_completion(nvshmem_transport_t transport,
    ...
    nvshmemt_libfabric_per_pe_comp_ring *comp_ring;
    ...
    comp_ring = &signal_state->proxy_put_signal_comp_map[pe];
    ...
    if (is_write_comp && nvshmemt_get_write_with_imm_hdr(entry->data) == NVSHMEMT_LIBFABRIC_IMM_STANDALONE_PUT_WITH_ACK_REQ) {
        ...
        comp_ring->insert(seq_num, ack_comp_entry);
    } else {
        nvshmemt_libfabric_comp_entry_t *existing = comp_ring->find(seq_num);
        if (existing) {
            if (!is_write_comp) existing->signal_entry.op = op;
            existing->signal_entry.progress_count += progress_count;
        } else {
            ...
            comp_ring->insert(seq_num, sig_comp_entry);
            existing = comp_ring->find(seq_num);
        }

        if (existing->signal_entry.progress_count != 0) {
            goto out;
        }
    }

    {
        ...
        while (true) {
            ...
            nvshmemt_libfabric_comp_entry_t *it = comp_ring->find(next_seq);
            ...
            if (it->type == NVSHMEMT_LIBFABRIC_COMP_ENTRY_SIGNAL) {
                ...
                if (it->signal_entry.op != NULL) {
                    auto op_ep = it->signal_entry.op->ep;
                    libfabric_state->op_queue[op_ep->domain_index]->putToRecv(
...
static int nvshmemt_libfabric_gdr_signal(struct nvshmem_transport *transport, int pe,
    ...
    do {
        status = libfabric_state->op_queue[ep->domain_index]->getNextSends((void **)(&context), 1);
    } while (try_again(transport, &status, &num_retries, qp_index,
                       NVSHMEMT_LIBFABRIC_TRY_AGAIN_CALL_SITE_GDR_SIGNAL_GET_NEXT_SENDS));
    ...
    do {
        status = fi_send(ep->endpoint, (void *)signal, sizeof(nvshmemt_libfabric_gdr_signal_op_t),
                         fi_mr_desc(libfabric_state->mr[ep->domain_index]), target_ep, &context->ofi_context);
    } while (try_again(transport, &status, &num_retries, qp_index,
                       NVSHMEMT_LIBFABRIC_TRY_AGAIN_CALL_SITE_GDR_SIGNAL_SEND));
...
static int nvshmemt_libfabric_connect_endpoints(nvshmem_transport_t t, int *selected_dev_ids,
    ...
    state->host_signal_state.n_pes = n_pes;
    state->host_signal_state.put_signal_seq_counter_per_pe.resize(n_pes);
    state->host_signal_state.proxy_put_signal_comp_map.resize(n_pes);
    state->host_signal_state.next_expected_seq.resize(n_pes, 0);

    state->proxy_signal_state.n_pes = n_pes;
    state->proxy_signal_state.put_signal_seq_counter_per_pe.resize(n_pes);
    state->proxy_signal_state.proxy_put_signal_comp_map.resize(n_pes);
    state->proxy_signal_state.next_expected_seq.resize(n_pes, 0);
    ...
    if (use_staged_atomics) {
        ...
        state->spawner_cpu = sched_getcpu();
